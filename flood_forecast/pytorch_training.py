...
def train_transformer_style(
    ...
    print("model_has_been_saved:",model_filepath)
   ...
   # print('cuda available: ',torch.cuda.is_available())
    ...
    for src, trg in data_loader:
       ...
       # print("step4")
        if takes_target:
           # print("step5")
            forward_params["t"] = trg.to(model.device)
       ...
       # print("step7.5")
       # print("fp", type(**forward_params))
        output = model.model(src, **forward_params).to(model.device)
        #print("step8")
...
def compute_validation(validation_loader: DataLoader,
    ...
    with torch.no_grad():
        ...
        for src, targ in validation_loader:
            #print("step6")
            #count_num_no_1 += 1
            #print("count num no 1:",count_num_no_1)
            ...
            if decoder_structure:
                ...
                if probabilistic:
                   # print("step10")
                    output_dist = model(src.float(),targ.float())
   ...
   # print("step15")
    model.train()
    print("i_is:",i)
    print("total_loss:",list(scaled_crit.values())[0]/float(i))
   # print("step16")
